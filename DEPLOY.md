...
2. In **Netlify → Site configuration → Environment variables**, add:
   - **`GROQ_API_KEY`** — your Groq key (the only thing required).
   - **`GROQ_MODEL`** — *(optional)* override the model id (default
     `llama-3.3-70b-versatile`).
3. Redeploy (or trigger a deploy). Open the **Ask AI** tab and ask away —
   e.g. *"Should I trade Bijan for CeeDee Lamb?"*

The key lives only in Netlify's environment — it's never sent to the browser,
so **paste it into Netlify, never into the app or anywhere else.** Without
`GROQ_API_KEY`, the tab shows a friendly "not configured yet" message and the
rest of the app is unaffected.

The chat endpoint is open (no password) and rate-limited. Since Groq's tier is
free that's usually fine; if it ever gets abused, re-adding a password gate is
a small change.
...
- The chat function only talks to Groq and is rate-limited.
